...
const jobSearch = new (0, _jobSearch.JobSearch)("#search-form", ".result-container");
...
class JobSearch {
    ...
    setCurrencyCode() {
        this.currencySymbol = (0, _utils.getCurrencySymbol)(this.countryCode); // Fix this line
    }
    setCurrencySymbol() {
    ...
    configureFormListener() {
        this.searchForm.addEventListener("submit", (event)=>{
            ...
            fetch(apiUrl).then((response)=>{
                ...
                else if (response.headers.get("content-type").includes("text/html")) return response.text(); // Handle HTML response
                else throw new Error("Unsupported content type");
            }).then((data)=>{
                console.log("After response, before processing data"); // Add this line
                if (typeof data === "string") // Handle HTML response
                this.resultsContainer.innerHTML = data;
                else {
                    // Handle JSON response
                    const jobs = data.results.map((job)=>(0, _templates.jobTemplate)(job, this.currencySymbol)).join("");
...
const jobTemplate = (jobs, currency)=>`
<div class="card">
   <div class="card-body">
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["hklly","bNKaB"], "bNKaB", "parcelRequire10c2")
